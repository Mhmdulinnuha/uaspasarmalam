...
class _PaymentPendingPageState extends State<PaymentPendingPage>
  ...
  Future<void> _launchGlobalInstitutePay() async {
    ...
    // Langsung coba launch tanpa canLaunchUrl karena:
    // - canLaunchUrl sering false-negatif di Android 11+ (package visibility)
    // - Lebih baik coba langsung dan handle error
    _log(' Mencoba launchUrl langsung (mode=externalApplication)...');
    try {
      ...
      if (launched) {
        ...
        _log('launchUrl=false — aplikasi tidak ditemukan atau tidak merespons');
        if (!mounted) return;
        ...
        children: [
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              icon: payStatus == PaymentCheckStatus.checking
                  ? SizedBox(
                      width: 18,
                      height: 18,
              ...
              label: Text(
                payStatus == PaymentCheckStatus.checking
                    ? 'Memproses...'
